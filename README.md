...
in the SML interface, represented as a character reader.  This uses the [reader]
(https://smlfamily.github.io/Basis/string-cvt.html#SIG:STRING_CVT.reader:TY)
...
CSV records and any validation of the content.  The MLB file `csv-reader.mlb`
provides this generic implementation.  

The structure `ListCsvReader` provides a CSV reader that represents the records
as `string list list`, as defined in the structure, `ListCsvTypes`, performing
no validation of the content.  This shows how the functor `CsvReader` can be
used.  The MLB file `list-csv-reader.mlb` provides this implementation.
...
val data =
  ListCsvReader.readRecords TextIO.StreamIO.input1 (TextIO.getInstream istream)
